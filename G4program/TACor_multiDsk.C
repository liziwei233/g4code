...
void TACor_multiDsk(const char *rootname=""){
	...
	int binu = (uR-uL)/1;
	const int T = 1;
	const int iter = 4; //the number of iteration
	//---------------------------------------------------//	
	//***************************************************//
	...
	TTree *t = (TTree*)f->Get("dsk");
	double xFIX[2][5];
	double xCFD[2][5];
	double amp[2];
	double ratio[5];
	double thrd[5];
	string pmtname[2]={"pmtL","pmtR"};
	
	double Utemp=0;
	double TCFDtemp=0,TFIXtemp=0;
	double TCFD_cor=0,TFIX_cor=0;
	float TT[1000000]={0.};

	//t->SetSetBranchAddressAddress("UL",&UL);
	//t->SetSetBranchAddressAddress("UR",UR);
	//t->SetSetBranchAddressAddress("T0L",&T0L);
	//t->SetSetBranchAddressAddress("T0R",T0R);
	//t->SetSetBranchAddressAddress("T0",T0);

	t->SetBranchAddress("amp",amp);	
	t->SetBranchAddress("T0_CFD",xCFD);	
	t->SetBranchAddress("CFD_fac",ratio);
	t->SetBranchAddress("T0_FIX",xFIX);	
	t->SetBranchAddress("FIX_fac",thrd);
	...
	TF1 *fitCFDAT = new TF1("fitCFDAT","0",uL,uR);
	TF1 *fitFIXAT = new TF1("fitFIXAT","0",uL,uR);
	//t->Draw("T0>>h");
	...
	for (int p=0;p<2;p++)
	{
		...
		for (int q=0;q<5;q++){
			...
			for(int s=0;s<iter;s++){
				...
				//Discriminator of constant factor;
				for(int i=0;i<N;i++){
					t->GetEntry(i);

					sprintf(name,"%s_%s_CFD_fac%g",rootname,pmtname[p].data(),ratio[q]);
					Utemp=amp[p];
					TCFDtemp=xCFD[p][q];
					TCFD_cor = fitCFDAT->Eval(Utemp);

					if(Utemp<0&&TCFDtemp!=0){
						if (s==0)
						{
							TT[i]=TCFDtemp-TCFD_cor;
						}
						else{
							TT[i] = TT[i]-TCFD_cor;
						}
				...
				fitCFDAT = profilefit(ht,rbu*2,rbt*2,tL,tR,uL,uR,buff);
				ht->Reset();
				h->Reset();
				hU->Reset();


				//Discriminator of FIX threshold;
				for(int i=0;i<N;i++){
					t->GetEntry(i);

					sprintf(name,"%s_%s_FIX_fac%gpe",rootname,pmtname[p].data(),thrd[q]);
					Utemp=amp[p];
					TFIXtemp=xFIX[p][q];
					TFIX_cor = fitFIXAT->Eval(Utemp);

					if(Utemp<0&&TFIXtemp!=0){
						if (s==0)
						{
							TT[i]=TFIXtemp-TFIX_cor;
						}
						else{
							TT[i] = TT[i]-TFIX_cor;
						}
				...
				//t->Draw("T0:UR>>ht","","colz");
				sprintf(buff,"%s_T0U_iter%d",name,s);
				fitFIXAT = profilefit(ht,rbu*2,rbt*2,tL,tR,uL,uR,buff);
				ht->Reset();
				h->Reset();
				//hU->Reset();
			}
        ...
        cout<<"===>> The procedure is complete! Thank You! <<===="<<endl;
}
