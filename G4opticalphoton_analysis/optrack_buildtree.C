...
void optrack_buildtree(){
    ...
    TFile *f1 = new TFile("optrack.root","RECREATE");
    ...
    TH1D *HB1 = new TH1D("B_abs","",nID,0,nID);
    TH1D *HB2 = new TH1D("B_hit","",nID,0,nID);

    TH1D *HL1 = new TH1D("L_abs","",nID,0,nID);
    TH1D *HL2 = new TH1D("L_hit","",nID,0,nID);

    TH1D *HW1 = new TH1D("wvl_abs","",200,100,9e3);
    TH1D *HW2 = new TH1D("wvl_hit","",200,100,9e3);

    TH1D *HT1 = new TH1D("T_abs","",2e2,0,3);
    TH1D *HT2 = new TH1D("Detected","",2e2,0,3);
    TH1D *HT = new TH1D("Total","",2e2,0,3);

    TH2D *H1 = new TH2D("Absorbed","",2e2,0,3,500,150,650);
    TH2D *H2 = new TH2D("hitpmt","",2e2,0,3,500,150,650);

    DrawMyHist1(HT1,"Time (ns)","Counts",4,2);
    DrawMyHist1(HT2,"Time (ns)","Counts",2,2);
    DrawMyHist1(HT,"Time (ns)","Counts",1,2);
    ...
    SetMyPad(c1, 0.12, 0.1, 0.1, 0.15);
    TCanvas *c2 = new TCanvas("c2","",1000,600);
    SetMyPad(c2, 0.12, 0.1, 0.1, 0.15);
    c1->cd();
    HT->Draw();
    //HT1->Draw("same");
    HT->SetLineStyle(7);
    HT2->Draw("SAMES");
    TLegend* leg1; 
    //leg1 = DrawMyLeg(0.12,0.75,0.35,0.88);
    leg1 = DrawMyLeg(0.6,0.2,0.75,0.32);
    leg1->AddEntry(HT2,"Detected","lp");
    leg1->AddEntry(HT,"Total","lp");
    leg1->Draw();

    gPad->Update();
    TPaveStats *stL1 = (TPaveStats*)HT2->FindObject("stats");
    ...
    TLegend* leg; 
    leg = DrawMyLeg(0.12,0.75,0.35,0.88);
    leg->AddEntry(H2,"Detected","lp");
