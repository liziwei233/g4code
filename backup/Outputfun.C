...
Double_t outputfunc(Double_t x, Double_t par[500]);
...
Double_t outputfunc(Double_t x, Double_t par[500]){
	...
	while(par[n]>5e-9){
		if (x-par[n]<-30e-9){
		val+=0;}
		else val+=response(x-tts-par[n],SPEpar);
...
void Outputfun(){
	...
	*/
	...
	sprintf(buff,"analysed_%s.root",name);
