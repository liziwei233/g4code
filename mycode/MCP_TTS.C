...
void MCP_TTS()
{
    ...
    //** end **
    //
    sfile = new TFile ("MCPcharsave.root","recreate");
    ...
    hx->GetXaxis()->SetRangeUser(10, 15);
...
*/
    ...
    //hq->Draw();
    //return;
    TH1* hqdc = lzw.SPSfit(hq, 4, chR.q, facspe);
    sfile->WriteTObject(hqdc);
    TF1 *fq = hqdc->GetFunction("myFun");
    //TF1* fq=lzw.mcpSPfit(hq,rbu,chR.q,facspe);
    //TF1 *fq = lzw.mcpSPfit(hq, rbu, chR.q, spefac1, spefac2);
...
*/
	...
	TF1* r1=twogausfit(hr,&rRL,&rRR,0.3,1);
	//return;
	double risetime=r1->GetParameter(1);
	//double pedqsigma=q1->GetParameter(2);
	//TF1* q2=gausfit(hq,1,qlimit1,qlimit2);
	...
	TF1* t=twogausfit(ht,&tRL,&tRR,0.2,2);
	...
	c[5]->cd();
	TF1* b1=twogausfit(hbl,&bRL,&bRR,0.4,1);
	double pedmean=b1->GetParameter(1)*1e3;
	double pedmeansigma=b1->GetParameter(2)*1e3;
	sprintf(buff,"%s_baseline.png",name);
	...
	c[6]->cd();
	TF1* b2=twogausfit(hblrms,&brmsRL,&brmsRR,0.4,1);
	double pedRMS=b2->GetParameter(1)*1e3;
	sprintf(buff,"%s_baselinerms.png",name);
